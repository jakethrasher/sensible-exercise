...
export const labelExtractor: LabelExtractor = (configuration: Label, text: StandardizedText) => {
  ...
  /**
   * Extract single and multi-line matches proximate to anchor point
   */
...
/**
 * Scans lines array starting from anchor point and looks for matches adjacent to the anchor line
 */
const findMatches = (
  configuration: Label,
  ...
  if (position === 'below' || position === 'above') {
    ...
    if (position === 'below') {
      /**
       * Iterate to the right over lines[] searching for matches on horizontal boundary.
    ...
    if (position === 'above') {
      /**
       * Iterate to the left over lines array searching for matches on horizontal boundary.
